...
public class PayloadPathUtil {
    ...
    public static Path moveWithSubdirIntact(Path inboundRoot, Path completeFilePath, Path destinationDir) {
        ...
        final Path newPathFor = PayloadPathUtil.createNewPathFor(inboundRoot, completeFilePath, destinationDir);

        // Performs the actual move
        final Path moved = PayloadPathUtil.move(completeFilePath, newPathFor);
        return moved;
    }
